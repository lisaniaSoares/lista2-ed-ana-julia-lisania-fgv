...
As transações realizadas são armazenadas em um **array dinâmico** de objetos `Transaction`, alocado com capacidade inicial para 200 transações. Um contador interno (`transactions_count`) rastreia quantas transações já foram registradas, permitindo o acesso e a cópia do histórico a qualquer momento por meio do método `getTransactions()`.
